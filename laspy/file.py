...
class File(object):
    ...
    def open(self):
        ...
        if self._mode in ("r", "r-"):
            ...
            if self._header is None:
                ...
                raise util.LaspyException("Headers must currently be stored in the file, you provided: {}".format(
                    self._header))
            ...
            self.add_extra_dimensions(self._reader.extra_dimensions)
            ...
            if isinstance(self._header, header.HeaderManager):
                ...
                if self._vlrs is not None:
                    self._vlrs.extend(vlrs)
                else:
                    self._vlrs = vlrs

                if self._evlrs is not None:
    ...
    def add_extra_dimensions(self, extra_dimensions):
