...
class DataProvider:
    ...
    def point_map(self, point_format):
        ...
        if self.mode == "r-":
            ...
            return
        pmap = self.get_point_map(point_format)
        if self.manager.header.version not in ("1.3", "1.4"):
            if self.manager.header.point_records_count != len(pmap):
                if self.manager.mode == "r":
                    raise laspy.util.LaspyException("""Invalid Point Records Count Information Encountered in Header.
                        ...
                        self.manager.header.point_records_count, len(pmap)))
                else:
...
class FileManager(object):
    ...
    def setup_read_write(self, vlrs, evlrs, read_only=True):
        ...
        self.pmap = self.data_provider.point_map(self.point_format)
    ...
    def get_extra_bytes(self):
        if "extra_bytes" in self.point_format.lookup.keys():
            ...
            newmap = self.data_provider.point_map(self.naive_point_format)
