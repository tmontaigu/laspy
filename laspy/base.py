...
class FakeMmap(object):
    ...
    def __init__(self, stream, compressed):
        stream.seek(0)
        if compressed:
            data = read_compressed(stream)
        else:
            data = stream.read()
...
class DataProvider:
    ...
    def map(self):
        ...
        if self.mode != "w":
            self._mmap = FakeMmap(self.fileref, self.compressed)
        else:
...
class FileManager(object):
    ...
    def __init__(self, filename, mode, header=None, vlrs=None, evlrs=None):
        ...
        self.mode = mode
        self.data_provider = DataProvider(filename, self.mode)
    ...
    def _read_words(self, fmt, num, bytes):
        ...
        outData = []
        for i in xrange(num):
            dat = self.data_provider._mmap.read(bytes)
    ...
    def _pack_words(self, fmt, num, val):
        if num == 1:
            return struct.pack(fmt, val)
        outData = b''.join(struct.pack(fmt, val[i]) for i in xrange(num))
        return outData

    def grab_file_version(self):
        """Manually grab file version from header"""
        self.data_provider._mmap.seek(24)
    ...
    def populate_evlrs(self):
        ...
        if self.header.version == "1.3":
            if self.header.start_wavefm_data_rec != 0:
                self.data_provider._mmap.seek(self.header.start_wavefm_data_rec)
                num_vlrs = 1
            else:
                num_vlrs = 0
        elif self.header.version == "1.4":
            self.data_provider._mmap.seek(self.header.start_first_evlr)
    ...
    def populate_vlrs(self):
        ...
        for i in xrange(self.get_header_property("num_variable_len_recs")):
            ...
            if self.data_provider._mmap.tell() > self.header.data_offset:
                self.data_provider._mmap.seek(self.header.data_offset)
    ...
    def get_raw_point(self, index):
        return self.pmap[index][0].tostring()
    ...
    def _get_raw_datum(self, rec_offs, spec):
        ...
        return self.data_provider._mmap[start: start + (spec.num * spec.length)]

    def _get_datum(self, rec_offs, spec):
        """Return unpacked data associated with non dimension field (VLR/Header)"""
        packed_data = self._get_raw_datum(rec_offs, spec)
        if spec.num == 1:
...
class Reader(FileManager):
    ...
    def get_scanner_channel(self):
        ...
        if not self.header.data_format_id in (6, 7, 8, 9, 10):
            raise laspy.util.LaspyException(
                "Scanner Channel not present for point format: " + str(self.header.data_format_id))
    ...
    def get_extra_bytes(self):
        if "extra_bytes" in self.point_format.lookup.keys():
            ...
            newmap = self.data_provider.point_map(self.naive_point_format, self.header)
...
class Writer(Reader):
    ...
    def define_new_dimension(self, name, data_type, description=""):
        ...
        if self.has_point_records or not self.mode == "w":
            raise laspy.util.LaspyException(
                "New dimensions may be defined only for write mode files which do not yet possess point records.")
        eb_vlrs = [x for x in self.vlrs if x.type == 1]
        if self.header.version == "1.4":
        ...
        if ("extra_bytes" in self.point_format.lookup) and (len(eb_vlrs) != 0):
            raise laspy.util.LaspyException("Adding a dimension is ambiguous when there are already extra bytes"
                                            "in the point records, but no VLR describing them.")
        ...
        if len(eb_vlrs) + len(eb_evlrs) > 1:
            ...
            if len(eb_vlrs) == 1:
                ...
                self.extra_dimensions = extra_dimensions
                new_pt_fmt = laspy.util.Format(
                    self.point_format.fmt, extradims=extra_dimensions)
                self.point_format = new_pt_fmt
                self.set_header_property("data_record_length", self.point_format.rec_len)
    ...
    def set_dimension(self, name, new_dim):
        ...
        if len(new_dim) != ptrecs:
            raise laspy.util.LaspyException(
                "Error, new dimension length ({}) does not match the number of points ({})".format(new_dim, ptrecs))
        try:
            spec = self.point_format.lookup[name]
            return self._set_dimension(spec, new_dim)
        except KeyError:
            raise laspy.util.LaspyException("Dimension: {} not found.".format(name))

    def _set_dimension(self, spec, value):
        self.pmap["point"][spec.name] = value
    ...
    def _set_raw_datum(self, rec_offs, spec, val):
        ...
        start = rec_offs + spec.offs
        self.data_provider._mmap[start:start + spec.num * spec.length] = val
    ...
    def _set_datum(self, rec_offs, dim, val):
        ...
        if dim.num != dimlen:
            raise (laspy.util.LaspyException(
                "Fields must be replaced with data of the same length. {} should be length  {} received {}".format(
                    dim.name, dimlen, dim.num)))

        for x in xrange(dim.num):
            try:
                # because val is a bytes() object, val[x] yields an integer in python 3,
                # while val[x:x+1] returns bytes in both python 2 and 3
                q = val[x:x + 1]
                out_byte = struct.pack(dim.fmt, q)
            except (TypeError, struct.error):
                out_byte = struct.pack(dim.fmt, int(val[x]))
            start = (x * dim.length) + rec_offs + dim.offs
            self.data_provider._mmap[start:start + dim.length] = out_byte
    ...
    def bitpack(self, arrs, indices):
        ...
        def keep_bits(arr, low, high):
            ...
            return ((arr & (2 ** high - 1)) & ~(2 ** low - 1)).astype(np.uint8)
        ...
        for arr, (low, high) in zip(arrs, indices):
            if low > first_bit_idx:
                packed += np.right_shift(keep_bits(arr, low, high), low - first_bit_idx)
            else:
                packed += np.left_shift(keep_bits(arr, low, high), first_bit_idx - low)
    ...
    def set_num_returns(self, num):
        ...
        if self.header.data_format_id in (0, 1, 2, 3, 4, 5):
            flag_byte = self.get_flag_byte()
            self.raise_if_overflow(num, 3)
            out_byte = self.bitpack((flag_byte, num, flag_byte), ((0, 3), (0, 3), (6, 8)))
            self.set_dimension("flag_byte", out_byte)
        elif self.header.data_format_id in (6, 7, 8, 9, 10):
            flag_byte = self.get_flag_byte()
            self.raise_if_overflow(num, 4)
            out_byte = self.bitpack((flag_byte, num), ((0, 4), (0, 4)))
            self.set_dimension("flag_byte", out_byte)

    def set_scanner_channel(self, value):
        if self.header.data_format_id not in (6, 7, 8, 9, 10):
            raise laspy.util.LaspyException(
                "Scanner Channel not present for point format: " + str(self.header.data_format_id))
        raw_dim = self.get_raw_classification_flags()
        self.raise_if_overflow(value, 2)
        out_byte = self.bitpack((raw_dim, value, raw_dim), ((0, 4), (0, 2), (6, 8)))
    ...
    def set_scan_dir_flag(self, flag):
        ...
        if self.header.data_format_id in (0, 1, 2, 3, 4, 5):
            ...
            out_byte = self.bitpack((flag_byte, flag, flag_byte),
                                    ((0, 6), (0, 1), (7, 8)))
            self.set_dimension("flag_byte", out_byte)
        elif self.header.data_format_id in (6, 7, 8, 9, 10):
            ...
            out_byte = self.bitpack((flag_byte, flag, flag_byte),
                                    ((0, 6), (0, 1), (7, 8)))
            self.set_dimension("classification_flags", out_byte)
    ...
    def set_edge_flight_line(self, line):
        ...
        if self.header.data_format_id in (0, 1, 2, 3, 4, 5):
            ...
            out_byte = self.bitpack((raw_dim, line), ((0, 7), (0, 1)))
            self.set_dimension("flag_byte", out_byte)
        elif self.header.data_format_id in (6, 7, 8, 9, 10):
            raw_dim = self.get_raw_classification_flags()
            self.raise_if_overflow(line, 1)
    ...
    def set_classification_flags(self, value):
        ...
        outbyte = self.bitpack((value, raw_dim), ((0, 4), (4, 8)))
