...
def compressed_fmt_to_uncompressed(fmt):
    return fmt & 0x3f
...
# Those specified here follow the byte size convention given in the
# LAS specification.
class Format:
    ...
    def __init__(self, fmt, overwritable=False, extra_bytes=False, extradims=[]):
        ...
        try:
            if is_point_fmt_compressed(int(fmt)):
                fmt = compressed_fmt_to_uncompressed(fmt)
            fmt = str(fmt)
        except ValueError:
            pass
        ...
        if fmt == "VLR":
            ...
            self.build_vlr_format()
        if fmt == "EVLR":
            self.format_type = "EVLR"
            self.build_evlr_format()
        # Header Fields
        if fmt[0] == "h":
