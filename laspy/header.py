...
class ParseableVLR:
    def __init__(self, user_id, record_id, vlr_body, description=None, reserved=None):
        self.user_id = str(user_id) + "\x00" * (16 - len(str(user_id)))
        self.record_id = record_id
        self.VLR_body = vlr_body
        self.rec_len_after_header = 0 if vlr_body is None else len(vlr_body)
        if description is not None:
            self.description = str(description) + "\x00" * (32 - len(description))
        else:
    ...
    def parse_data(self):
        ...
        elif "LASF_Spec" in self.user_id and 99 < self.record_id < 355:
...
class EVLR(ParseableVLR):
    ...
    def __init__(self, user_id, record_id, vlr_body=None, description=None, reserved=None):
